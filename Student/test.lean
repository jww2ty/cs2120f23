...
def glue_funs {α β γ : Type} (g : β → γ) (f : α → β) (s : α) : γ :=
  g (f s)
...
#eval glue_funs double square 5 -- Expected result = 50
#eval glue_funs square double 5 -- Expected result = 100

-- In both cases the second function is applied first, and the first function second. 
--Therefore for case 1, 5 is squared and then doubled, resulting in 50
--For case 2, 5 is doubled and then squared, resulting in 100
--Thus, they return different results because the order of which the double and square functions are applied matters in terms of end result.
