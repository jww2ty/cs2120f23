...
def its_wet (rain sprinkler wet : Type) (rainsprinkler : rain ⊕ sprinkler) (rainTowet : rain → wet) (sprinklerTowet : sprinkler → wet) : wet :=
  match rainsprinkler with
  | Sum.inl r => rainTowet r
  | Sum.inr s => sprinklerTowet s
...
def sum_elim {α β γ : Type} (αβ : α ⊕ β) (αToγ : α → γ) (βToγ : β → γ) : γ :=
  match αβ with
  | Sum.inl a => αToγ a
  | Sum.inr b => βToγ b
